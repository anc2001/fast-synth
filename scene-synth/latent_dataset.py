...
def create_transformed_composite(scene, objects, rot):
    ...
    if rot != 0:
        ...
        rot_mat = np.array([[c, 0, -s, 0], [0, 1, 0, 0], [s, 0, c, 0], [0, 0, 0, 1]])
        # The full process is: translate center of scene to origin, rotate, translate back
        hw = scene.size / 2
        trans_mat1 = np.array(
            [[1, 0, 0, -hw], [0, 1, 0, 0], [0, 0, 1, -hw], [0, 0, 0, 1]]
        )
        trans_mat2 = np.array(
            [[1, 0, 0, hw], [0, 1, 0, 0], [0, 0, 1, hw], [0, 0, 0, 1]]
        )
...
class LatentDataset(data.Dataset):
    ...
    def __init__(
        ...
        with open(
            f"{self.data_root_dir}/{self.data_folder}/final_categories_frequency", "r"
        ) as f:
            lines = f.readlines()
            names = [line.split()[0] for line in lines]
            names = [
                name for name in names if ((name != "door") and (name != "window"))
            ]
            self.catnames = names
            ...
            self.cat2freq_normalized = {
                cat: freq / maxfreq for cat, freq in self.cat2freq.items()
            }
    ...
    def build_cats_in_scene_indices(self):
        ...
        for cat, scene_indices in self.cat_index2scenes.items():
            scenes = [
                ...
                if (idx >= self.scene_indices[0] and idx < self.scene_indices[1])
            ]
            if len(scenes) > 0:
                cats_seen[cat] = True
    ...
    def sort_object_nodes_by_importance(self, object_nodes, noise=None, swap_prob=None):
        ...
        if noise is not None:
            ...
            index_imp_pairs = [
                (index, imp + noise * random.normalvariate(0, istd))
    ...
    def order_object_nodes(self, object_nodes):
        ...
        if "parent" in object_nodes[0]:
            ...
            def is_second_tier(node):
                return (node["parent"] != "Wall") and (node["parent"] != "Floor")
    ...
    def get_scene_specific_category(self, cat_index_or_name, empty_room=False):
        ...
        scenes_for_cat = [
            idx
            for idx in self.cat_index2scenes[cat_index]
            if (idx >= self.scene_indices[0] and idx < self.scene_indices[1])
        ]
    ...
    def get_scene_uniform_category(self, stop_prob=None):
        if stop_prob is not None and random.random() < stop_prob:
            scene_index = random.randint(
                self.scene_indices[0], self.scene_indices[1] - 1
            )
            scene = RenderedScene(scene_index, self.data_folder, self.data_root_dir)
            output_node = None
...
class SameCategoryBatchSampler(object):
    ...
    def __init__(self, n_categories, epoch_size, batch_size, drop_last=False):
        if (
            ...
            raise ValueError(
                "n_categories should be a positive integeral value, "
                "but got n_categories={}".format(n_categories)
            )
        if (
            not isinstance(epoch_size, int)
            or isinstance(epoch_size, bool)
            or batch_size <= 0
        ):
            raise ValueError(
                "epoch_size should be a positive integeral value, "
