...
def toilet_filter(version, room_size, second_tier, source):
    ...
    if version == "set1":
        ...
        def node_criteria(node, room):
            ...
            if (
                node.parent
                and node.parent != "Floor"
                and not coarse_category in no_count_object_categories
            ):
                return False
            if coarse_category in rejected_object_categories:
                return True
            if coarse_categories_frequency[coarse_category] < freq_threshold:
                return False
            if coarse_category in filtered_object_categories:
                return False
            if node.length * node.width < 0.05:
                return False
            return True
            ...
            for node in room.nodes:
                ...
                if not coarse_category in no_count_object_categories:
                    if node.zmin - room.zmin > 0.1:
                        return False
                    category_dict[coarse_category] = (
                        category_dict.get(coarse_category, 0) + 1
                    )
                    true_node_count += 1

            if true_node_count < 4 or true_node_count > 20:
                return False

            if not os.path.isfile(
                f"./data/suncg_data/room/{room.house_id}/{room.modelId}f.obj"
            ):
                return False
            if not os.path.isfile(
                f"./data/suncg_data/room/{room.house_id}/{room.modelId}w.obj"
            ):
                return False
        ...
        def room_criteria(room, house):
            ...
            for node in room.nodes:
                ...
                if frequency[category] < 200:
                    return False

            if node_count < 3 or node_count > 10:
                return False
            if toilet_count > 2:
                return False
            if bathtub_count > 3:
        ...
        def node_criteria(node, room):
            ...
            if second_tier:
                ...
                if node.parent:
                    if (
                        isinstance(node.parent, Node)
                        and node.zmin < node.parent.zmax - 0.1
                    ):
                        return False
                    node_now = node
            ...
            for i in range(5):
                room.nodes = [
                    ...
                    if not (
                        (
                            node.parent
                            and isinstance(node.parent, Node)
                            and (node.parent) not in room.nodes
                        )
                    )
                ]
            for node in room.nodes:
                ...
                if not category in door_window:
                    ...
                    if (
                        not 0.9 < xscale < 1.1
                    ):  # Reject rooms where any object is scaled by too much
                        return False
                ...
                if frequency[category] < 500:
                    return False

            # if not scaled:
            # return False
            if floor_node_count < 4 or node_count > 20:
                return False
