...
def sample_orientation(orient_model, input_img, category):
    ...
    try:
        ...
        temp_noise = torch.cat([noise, torch.clone(noise)], dim=0)
        temp_img = torch.cat([input_img, torch.clone(input_img)], dim=0)
        return orient_model.generate(temp_noise, temp_img, category)[0:1]
...
def sample_dimensions(dims_model, input_img, category):
    ...
    try:
        ...
        temp_noise = torch.cat([noise, torch.clone(noise)], dim=0)
        temp_img = torch.cat([input_img, torch.clone(input_img)], dim=0)
        return dims_model.generate(temp_noise, temp_img, category)[0:1]
